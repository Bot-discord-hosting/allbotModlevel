...
class Levelsys(commands.Cog):
      ...
      if not message.author.bot:
        ...
        if str(message.guild.id) in data:
          if str(message.author.id) in data[str(message.guild.id)]:
            ...
            if new_level > lvl:
              await message.channel.send(f"{message.author.mention} مبروك لقد إرتفع مستواك {new_level}🎉🎉")
              ...
              for i in range(len(level)):
                if new_level == level_num[i]:
                  ...
                  mbed = nextcord.Embed(title=f"{message.author} You Have Gotten role **{level[i]}**!", color = message.author.colour)
  ...
  async def id(self, ctx: commands.Context, user: Optional[nextcord.Member]):
  ...
  async def rank_reset(self, ctx, user: Optional[nextcord.Member]):
    ...
    if not member == ctx.author:
      role = nextcord.utils.get(ctx.author.guild.roles, name="Bot-Mod")
  ...
  async def increase_level(self, ctx, increase_by: int, user: Optional[nextcord.Member]):
    member = user or ctx.author
  ...
  @commands.has_role("Bot-Mod")
  async def increase_xp(self, ctx, increase_by: int, user: Optional[nextcord.Member]):
    member = user or ctx.author
